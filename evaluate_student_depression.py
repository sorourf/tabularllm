"""
TabLLM Evaluation Script for Student Depression Dataset
...
from helper.external_datasets_variables import (
    ...
    template_config_student_depression_list
)
...
def load_student_depression_data(data_dir):
    """Load and preprocess student depression dataset"""
    logger.info("Loading student depression dataset...")

    # Load train and test data
    train_df = pd.read_csv(data_dir / 'train_student_depression_sample.csv')
    test_df = pd.read_csv(data_dir / 'test_student_depression_sample.csv')

    logger.info(f"Training samples: {len(train_df)}, Test samples: {len(test_df)}")

    # Rename columns to match template variable names (remove spaces and special chars)
    column_mapping = {
        'Depression': 'label',
...
def train_and_evaluate_classifier(train_texts, train_labels, test_texts, test_labels, output_dir):
    ...
    # Print results
    logger.info("\n" + "="*80)
    logger.info("TABLLM EVALUATION RESULTS - Student Depression Classification")
    logger.info("="*80)
    ...
    logger.info("\n" + classification_report(test_labels, y_pred, target_names=['No Depression', 'Depression']))
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate TabLLM on Student Depression Dataset")
    parser.add_argument(
        "--data_dir",
        type=str,
        default="testdata",
    ...
    train_df, test_df = load_student_depression_data(data_dir)
